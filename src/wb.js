...
var socket, settings, config, $messages, $msg, drawings = [], canvas, availableId
...
function setupSockets(){
    ...
    socket.on('id', (d) => { availableId =d })
    socket.on( 'erase', () =>{ canvas.clear() })
}
...
function setupUIEvents() {
    ...
    $('#edit').on('click',function(){
        ...
        if (current) { $t.addClass('selected') } else { $t.removeClass('selected') }
    })
    ...
    $('#color').spectrum({
        ...
        palette: [
            ...
            ['lightblue','lightgreen','pink','violet','orange'],
            ['rgb(225,26,43)', 'rgb(164,18,20)','rgb(89,44,130)']
        ],
    ...
    $('#erasedraw').on('click',function(){
        ...
        if (settings.draw.isErase) { $t.addClass('selected') } else { $t.removeClass('selected') }
    })
...
function sendMsg(msg) {
    ...
    settings.chat.to = config.users.other
    settings.chat.from = settings.id
    settings.type = config.types.chat
    socket.emit('send', settings)
}
...
function setupCanvas(){
    ...
    canvas.on('path:created', function(e){
        settings.type = config.types.draw
        settings.draw = e
    ...
    canvas.on('object:modified', function(e){
        settings.type = config.types.update
        ...
        /*
            canvas.on('mouse:down', function(e){
                if (settings.type === config.types.text) {
